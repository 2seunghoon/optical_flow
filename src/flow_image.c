    ...
    for (int c = 0; c < im.c; c++) {
        for (int row = 0; row < im.h; row++) {
            for (int col = 0; col < im.w; col++) {
                float pixel_val = get_pixel(im, col, row, c);
                if (col > 0) {
                    pixel_val += get_pixel(integ, col - 1, row, c);
                }
                if (row > 0) {
                    pixel_val += get_pixel(integ, col, row - 1, c);
                }
                if (col > 0 && row > 0) {
                    pixel_val -= get_pixel(integ, col - 1, row - 1, c);
                }
                set_pixel(integ, col, row, c, pixel_val);
            }
        }
    }
...
image box_filter_image(image im, int s)
{
    ...
    for (k = 0; k < integ.c; k++) {
        for (j = 0; j < integ.h; j++) {
            for (i = 0; i < integ.w; i++) {
                int offset = s / 2;
                float sum_val = get_pixel(integ, i + offset, j + offset, k) + get_pixel(integ, i - offset - 1, j - offset - 1, k) - (get_pixel(integ, i - offset - 1, j + offset, k) + get_pixel(integ, i + offset, j - offset - 1, k));
                sum_val /= (s * s);
                set_pixel(S, i, j, k, sum_val);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    for (int row = 0; row < im.h; row++) {
        ...
        for (int col = 0; col < im.w; col++) {
            ...
            set_pixel(S, col, row, 0, get_pixel(ix, col, row, 0) * get_pixel(ix, col, row, 0));
            set_pixel(S, col, row, 1, get_pixel(iy, col, row, 0) * get_pixel(iy, col, row, 0));
            set_pixel(S, col, row, 2, get_pixel(ix, col, row, 0) * get_pixel(iy, col, row, 0));
            set_pixel(S, col, row, 3, get_pixel(ix, col, row, 0) * get_pixel(it, col, row, 0));
            set_pixel(S, col, row, 4, get_pixel(iy, col, row, 0) * get_pixel(it, col, row, 0));
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            if (invert_M.data) {
                vx = invert_M.data[0][0] * (-Ixt) + invert_M.data[0][1] * (-Iyt);
                vy = invert_M.data[1][0] * (-Ixt) + invert_M.data[1][1] * (-Iyt);
            }
